...
# To get te max price the ownner can get
def maxPrice(inputlist): 
    ...
    for i in range(0,len(uniqueDileveryCost)):
        q = getIndexPositions(deliveryCostCopy,uniqueDileveryCost[i]) #from uniques delivery cost we compare the main deliver cost list to see if any value is beeing repeated or not
...
if validInput:
    try: 
        inputFileContents = totalInputs.copy() # Reading the contents of the file
